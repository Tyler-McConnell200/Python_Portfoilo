...
y2 = avgs.revenue
fig, ax = plt.subplots()
plot(x, y2, ax, 'Increase in mean Fortune 500 companyfrom 1955 to 2005', 'Revenue (millions)')
```
...
title = 'Increase in mean and std forunte 500 company %s from 1955 to 2005'
...
```python
# We can name a function inside another function
...
patient_0 = data[0, :] #) on the first axis(rows), everything on the second (columns)
...
```
    ...
    final value of name: ['Curie', 'Darwin', 'Turing']
...
```
    ...
    odds after removing the first element: [7, 11]
    removed_element: 5
...
print('Using len() to last entry:', sond)
sond = months[8:]
print('Omitting ending index:', sond)
```

    With known last position: ['sep', 'oct', 'nov', 'dec']
    Using len() to last entry: ['sep', 'oct', 'nov', 'dec']
...
```
    ...
    <ipython-input-4-8c31eec6205b> in <module>
          3 print(odds[1])
          4 print(odds[2])
    ----> 5 print(odds[3])
          6 
...
if max_inflammation_0 == 0 and max_inflammation_20 ==20:
    print('suspictious looking maxima!')
elif numpy.sum(numpy.amin(data, axis=0)) ==0:
...
def fahr_to_celsius(temp):
    ...
    # explicit in explaining how the return command workds.
    return((temp - 32) * (5/9))
```
...
print('Temperature in Kelvin was:', temp_kelvining)
```
...
# return a new array containting the original data with its mean offset to match the desired value.
...
```python
def offset_mean(data, target_mean_value):
    """"Return a new array containg the original data 
...
total = 0.0
for num in numbers:
    assert num > 0.0, 'Data should only contail positive values'
    total += num
...
def normalize_rectangle(rect):
    """Normalize a rectangel so that it is at the angle and 1.0 units long on its longest axis.
...
```
    ...
    <ipython-input-26-36b45d872f48> in <module>
    ----> 1 print(normalize_rectangle(4.0, 2.0, 1.0, 5.0))
    

    TypeError: normalize_rectangle() takes 1 positional argument but 4 were given
...
```python
print(normalize_rectangle( (0.0, 0.0, 5.0, 2.0)))
```
...
```
    ...
    Enter the name of the input fasta file:  P53_gene.fasta
...
with open(input_file_name, "r") as input_file:
    ...
    for line in input_file:
        if line.startswith(">"):
            continue
            dna_sequence += line.script()
```


```python
# Trancribe the DNA to RNA
...
```
    ...
    <ipython-input-9-f772c4750a12> in <module>
          1 # Save the RNA sequence to a text file
    ----> 2 with open(output_file_name, "w") as output_file:
          3     output_file.write(rna_sequence)
          4     print(f"The RNA sequence has been saved to {output_file_name}")
...
```python
input_file_name = input("Enter the name of the input file RNA file:")
```
...
```
    ...
    The protein sequence has been saved to out.txt
